...
public:
	...
	explicit GraphVertex(const shared_ptr<ISceneNode> & node);
	void ClearState();
...
public:
	...
	shared_ptr<GraphVertex> GetVertexNearestTo(Vector3 position, int * vertexNum = nullptr);
};
...
public:
	...
	Path(shared_ptr<ISceneNode> scene, string pathBaseName);
	vector<shared_ptr<GraphVertex>> mVertexList;
	void AddPointAndLinkWithPrevious(const shared_ptr<GraphVertex> & vertex);
	class NodeSorter {
	public:
		bool operator() (const shared_ptr<ISceneNode> node1, const shared_ptr<ISceneNode> node2);
	};
